...
<div id="filterPanel" class="fixed inset-y-0 right-0 z-40 w-full max-w-sm bg-white border-l border-gray-200 shadow-xl hidden">
    ...
    <form method="GET" action="{{ route('purchases.index') }}" class="p-4 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="ref" value="{{ request('ref') }}" class="w-full border rounded px-3 py-2" placeholder="Item">
        </div>
        <div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-100">
                    ...
                    <td class="py-2">
@php
    $img = 'storage/images/purchases/no-image.png';
    if (!empty($p->image)) {
        $img = 'storage/images/purchases/' . trim(explode(',', $p->image)[0]);
    }
@endphp
<img src="{{ asset('images/purchases/' . $p->image) }}" width="50">
