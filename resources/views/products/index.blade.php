...
<div class="space-y-3">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
        ...
        <table class="min-w-[900px] w-full table-auto">
            ...
            <tbody id="productsTable" class="divide-y divide-gray-100 text-[13px]">
                @php $rows = $products ?? []; @endphp
                @if(empty($rows))
                    <tr><td colspan="12" class="px-4 py-8 text-center text-gray-500">No products found</td></tr>
                @else
                    @foreach($rows as $p)
    ...
    <div class="flex items-center justify-between text-sm text-gray-600">
        ...
        <div>1 - {{ min(10, count($rows)) }} of {{ count($rows) }}</div>
        <div class="flex items-center gap-4">
            <button class="text-gray-400" disabled>prev</button>
            <button class="text-violet-600">next</button>
        </div>
    </div>
</div>
...
document.getElementById('approveBtn').addEventListener('click', async () => {
    ...
    const res = await fetch('{{ route('products.approve') }}', { method:'POST', headers:{ 'Content-Type':'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') }, body: JSON.stringify({ product_ids: ids }) });
