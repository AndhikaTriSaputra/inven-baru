    ...
    <form method="POST" action="{{ route('transfers.update', $transfer->id) }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div>
                    ...
                    <div class="relative">
                        <input type="date" name="date" value="{{ old('date', $transfer->date) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-violet-500 focus:border-transparent" required>
            ...
            <div>
                    ...
                    @foreach($warehouses as $wh)
                        <option value="{{ $wh->id }}" {{ $wh->id == $transfer->from_warehouse_id ? 'selected' : '' }}>{{ $wh->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                    ...
                    <option value="">Choose Warehouse</option>
                    @foreach($warehouses as $wh)
                        <option value="{{ $wh->id }}" {{ $wh->id == $transfer->to_warehouse_id ? 'selected' : '' }}>{{ $wh->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        {{-- Product Table --}}
                ...
                <tbody id="transfer-product-list">
                    @if($transferDetails->count() > 0)
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <div class="space-y-2">
                                    <div class="text-sm font-medium text-gray-900">{{ $detail->product_code ?? 'N/A' }}</div>
                                    <div class="flex items-center gap-2">
                                        <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-800">
                                            {{ $detail->product_name }}
                                        </span>
                                        <button type="button" onclick="openProductModal({{ $index }}, {{ $detail->product_id }})" class="w-4 h-4 text-gray-400 hover:text-gray-600">
                                ...
                                <input type="hidden" name="items[{{ $index }}][purchase_unit_id]" value="{{ $detail->purchase_unit_id }}">
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-1">
                                    ...
                                    <input type="number" name="items[{{ $index }}][quantity]" id="qty-{{ $index }}" class="w-16 px-2 py-1 text-center border border-gray-300 rounded focus:ring-2 focus:ring-violet-500 focus:border-transparent" value="{{ $detail->quantity }}" min="0" step="1" required>
            ...
            <div>
                    ...
                    <select name="statut" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-violet-500 focus:border-transparent" required>
                        <option value="completed" {{ $transfer->statut == 'completed' ? 'selected' : '' }}>Completed</option>
                    <option value="pending" {{ $transfer->statut == 'pending' ? 'selected' : '' }}>Pending</option>
                    <option value="sent" {{ $transfer->statut == 'sent' ? 'selected' : '' }}>Sent</option>
                </select>
            </div>
        </div>
        ...
        {{-- Notes --}}
            <div class="mb-6">
                ...
                <textarea name="notes" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-violet-500 focus:border-transparent" rows="3" placeholder="A few words ...">{{ $transfer->notes }}</textarea>
            </div>
        </div>
...
let addedItems = @json($transferDetails);
    ...
    if (addedItems.length === 0) {
        ...
        addedItems.forEach((item, idx) => {
            ...
            row.innerHTML = `
                ...
                <td class="px-4 py-3">
                    <div class="space-y-2">
                        <div class="text-sm font-medium text-gray-900">${item.product_code || item.code || 'N/A'}</div>
                        <div class="flex items-center gap-2">
                            <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-800">
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center space-x-1">
                        ...
                        <input type="number" name="items[${idx}][quantity]" id="qty-${idx}" class="w-16 px-2 py-1 text-center border border-gray-300 rounded focus:ring-2 focus:ring-violet-500 focus:border-transparent" value="${item.quantity || 0}" min="0" step="1" required>
